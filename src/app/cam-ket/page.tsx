...
export const metadata: Metadata = {
  title: "Cam kết đào tạo",

  description:
    "Tìm hiểu các cam kết đào tạo, phương pháp giảng dạy và định hướng học tập tại Crown English.",
};
    ...
    <main className="min-h-screen bg-transparent text-foreground">
      ...
      <section className="bg-background pt-32 pb-20 lg:pt-40">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {courseCommitments.map((course) => (
                <Link
                  key={course.slug}
                  href={`#${course.slug}`}
                  className="rounded-full border border-line bg-surface px-5 py-2.5 text-sm font-bold transition hover:border-primary hover:text-accent"
                >
                  {course.title}
                </Link>
              ))}
        ...
        <section className="pb-16" aria-labelledby="course-commitments">
          <Container>
            <h2
              id="course-commitments"
              className="text-3xl font-bold leading-tight sm:text-4xl"
            >
              {commitmentPage.coursesTitle}
            </h2>
            <p className="mt-4 max-w-2xl leading-7 text-muted">
              {commitmentPage.coursesDescription}
            </p>
            <div className="mt-8 grid gap-6 lg:grid-cols-2">
              {courseCommitments.map((course) => (
                <article
                  key={course.slug}
                  id={course.slug}
                  className="flex scroll-mt-28 flex-col rounded-[26px] border border-line bg-surface p-6 shadow-sm sm:p-8"
                >
                  ...
                  <dl className="mt-6 grid gap-3 sm:grid-cols-3">
                    {course.outcomes.map((outcome) => (
                      ...
                      >
                        ...
                        <dd className="mt-2 text-xl font-bold leading-snug text-accent">
                          {outcome.value}
                        </dd>
                      </div>
                    ))}
                  ...
                  <div className="mt-6">
                    <h4 className="font-bold">
                      {commitmentPage.assessmentLabel}
                    </h4>
                  ...
                  <div className="mt-auto pt-6">
                    <Link
                      ...
                      className="inline-flex font-bold text-accent transition hover:translate-x-1"
                    >
                      {commitmentPage.courseLinkLabel}
                    </Link>
                  </div>
                </article>
              ))}
        ...
        <section className="pb-16" aria-labelledby="commitment-conditions">
          <Container className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
            <div>
              <p className="mb-4 text-xs font-bold uppercase tracking-[0.22em] text-accent">
                {commitmentPage.conditionsEyebrow}
              </p>
              <h2
                id="commitment-conditions"
                className="text-3xl font-bold leading-tight sm:text-4xl"
              >
                {commitmentPage.conditionsTitle}
              </h2>
              <p className="mt-5 text-lg leading-8 text-muted">
                {commitmentPage.conditionsDescription}
              </p>
            </div>
        ...
        <section aria-labelledby="commitment-quality">
          <Container>
            ...
            >
              {commitmentPage.qualityTitle}
            </h2>
            <div className="mt-8 grid gap-6 md:grid-cols-2">
              {commitmentValues.map((value) => (
                <article
                  key={value.title}
                  className="rounded-[26px] border border-line bg-surface p-6 sm:p-8"
                >
                  <span
                    className="block h-1 w-12 rounded-full brand-panel bg-primary"
                    aria-hidden="true"
                  />
            ...
            <div className="mt-10 flex flex-col gap-6 rounded-[28px] brand-panel bg-primary p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
              <div className="max-w-2xl">
                <h2 className="text-2xl font-bold leading-tight sm:text-3xl">
                  {commitmentPage.ctaTitle}
                </h2>
                <p className="mt-3 leading-7">
                  {commitmentPage.ctaDescription}
                </p>
              </div>
              <Link
                href={commitmentPage.cta.href}
                className="inline-flex w-fit shrink-0 rounded-full brand-panel bg-primary px-7 py-3.5 text-center font-bold text-foreground transition hover:-translate-y-1"
              >
                {commitmentPage.cta.label}
              </Link>
            </div>
          </Container>
        </section>
