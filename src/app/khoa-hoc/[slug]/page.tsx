...
export function generateStaticParams() {
  return courses.flatMap((course) =>
    [course.slug, ...(course.aliases ?? [])].map((slug) => ({ slug })),
  );
}
    ...
    <main className="min-h-screen bg-transparent text-foreground">
      ...
      <section className="bg-background pt-32 pb-20 lg:pt-40">
        <Container>
          <Link
            href="/khoa-hoc"
            className="group inline-flex min-h-11 items-center gap-2.5 rounded-full border border-line bg-surface px-5 py-2.5 text-base font-semibold text-foreground transition-colors hover:border-primary hover:bg-tint focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
          >
            <ArrowLeft
              size={20}
              strokeWidth={1.8}
          ...
          <div className="mt-8 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
            <div>
              <p className="mb-5 inline-flex rounded-full border border-primary/30 bg-primary/10 px-4 py-2 text-base font-bold tracking-wide text-accent">
                {course.tag}
              </p>
              ...
              <Link
                href={{ pathname: "/lien-he", query: { course: course.slug } }}
                className="mt-7 inline-flex rounded-full brand-panel bg-primary px-7 py-3.5 font-bold text-foreground transition hover:-translate-y-1 hover:bg-primary-hover"
              >
          ...
          <section className="mt-16" aria-labelledby="course-roadmap">
            ...
            <ol className="mt-8 grid gap-6">
              {course.stages.map((stage, index) => (
                <li
                  key={stage.title}
                  className="rounded-[26px] border border-line bg-surface p-6 shadow-sm sm:p-8"
                >
                  <div className="grid gap-6 lg:grid-cols-[0.7fr_1.3fr]">
                    <div>
                      <span className="grid size-10 place-items-center rounded-full brand-panel bg-primary font-bold">
          ...
          {course.notes.length > 0 && (
            <section
              className="mt-8 rounded-[26px] border border-line p-6 sm:p-8"
              aria-labelledby="course-notes"
            >
              <h2 id="course-notes" className="text-xl font-bold">
                Thông tin học phí & chính sách
              </h2>
